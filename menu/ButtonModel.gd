...
func _process(_delta):
	# Handling focus:
	if has_focus():
		if main.control_handler.get_directional_input().y == 1 and $Timer.is_stopped():
			get_node(focus_neighbour_bottom).grab_focus()
		elif main.control_handler.get_directional_input().y == -1 and $Timer.is_stopped():
			get_node(focus_neighbour_top).grab_focus()
		elif main.control_handler.get_directional_input().x == 1 and $Timer.is_stopped():
